...
pub struct NetworkState {
    ...
    pub competitor_activity: HashMap<Pubkey, Vec<u64>>,
...
impl ComputeUnitAuctionEngine {
    ...
    fn adjust_compute_units(&self, simulated: u32) -> u32 {
        let buffer_factor = 1.15;
        let adjusted = (simulated as f64 * buffer_factor) as u32;
        adjusted.max(MIN_COMPUTE_UNITS).min(MAX_COMPUTE_UNITS)
    }
    ...
    async fn calculate_priority_fee(
        ...
        let network_state = self.network_state.read().unwrap();
        let strategy = self.strategy.read().unwrap();
        
        let base_fee = self.get_dynamic_base_fee(&network_state, &strategy);
        ...
        let fee = (base_fee as f64 
            ...
            * adaptive_factor
            * strategy.aggression_factor) as u64;
    ...
    fn get_dynamic_base_fee(&self, network_state: &NetworkState, strategy: &AuctionStrategy) -> u64 {
        ...
        let percentile_idx = ((network_state.average_priority_fees.len() as f64 
            * PERCENTILE_TARGET) as usize).min(network_state.average_priority_fees.len() - 1);
    ...
    pub async fn update_network_state(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mut network_state = self.network_state.write().unwrap();
        network_state.current_slot = slot;
        
        if let Some(rate) = block_production.value.by_identity.values().next() {
            network_state.recent_block_production_rate = 
                rate.0 as f64 / (rate.0 + rate.1) as f64;
        }
    ...
    pub async fn record_transaction_result(
        ...
        let metric = ComputeUnitMetrics {
            ...
            success,
            latency_ms,
            network_congestion: self.network_state.read().unwrap().congestion_score,
    ...
    fn update_metrics_history(&self, metric: ComputeUnitMetrics) {
        let mut history = self.metrics_history.write().unwrap();
        history.push_back(metric.clone());
        
        if history.len() > SLOT_HISTORY_SIZE {
            history.pop_front();
        }
        
                    self.performance_cache.insert(metric.slot, 
                if metric.success { 1.0 } else { 0.0 });
        
        let mut network_state = self.network_state.write().unwrap();
        network_state.average_priority_fees.push_back(metric.priority_fee);
        if network_state.average_priority_fees.len() > SLOT_HISTORY_SIZE {
            network_state.average_priority_fees.pop_front();
        }
    }
    ...
    fn update_fee_percentiles(&self) {
        ...
        let percentiles = vec![
            fees[fees.len() * 50 / 100],
            fees[fees.len() * 75 / 100],
            fees[fees.len() * 90 / 100],
            fees[fees.len() * 95 / 100],
            fees[fees.len() * 99 / 100],
        ];
        
        *self.fee_percentiles.write().unwrap() = percentiles;
    }
    ...
    pub async fn get_aggressive_bid(
        ...
        let percentiles = self.fee_percentiles.read().unwrap();
    ...
    pub async fn optimize_for_latency(
        ...
        let low_latency_fees: Vec<u64> = history.iter()
            .filter(|m| m.success && m.latency_ms <= target_latency_ms)
            .map(|m| m.priority_fee)
            .collect();
        
        if low_latency_fees.is_empty() {
            return self.get_aggressive_bid(0.9).await;
        }
        
        let optimal_fee = low_latency_fees.iter().sum::<u64>() / low_latency_fees.len() as u64;
        let latency_premium = 1.0 + (50.0 / target_latency_ms as f64).min(0.5);
        
        let final_fee = (optimal_fee as f64 * latency_premium) as u64;
        ...
        if let Ok(tx) = self.rpc_client.get_transaction(
            ...
            if let Some(meta) = tx.transaction.meta {
                if let Some(fee) = meta.fee {
                    ...
                    if let Some(fees) = network_state.competitor_activity.get_mut(competitor_pubkey) {
                        if fees.len() > 20 {
                            fees.remove(0);
                        }
                    }
...
impl ComputeUnitAuctionEngine {
    ...
    pub fn validate_compute_budget(&self, units: u32, fee: u64) -> bool {
        units >= MIN_COMPUTE_UNITS && 
        units <= MAX_COMPUTE_UNITS && 
        fee >= BASE_PRIORITY_FEE && 
        fee <= MAX_PRIORITY_FEE
    }
    ...
    pub async fn get_market_maker_bid(
        &self,
        spread_bps: u64,
        volume: u64,
    ) -> Result<(u32, u64), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let base_fee = self.get_dynamic_base_fee(&network_state, &self.strategy.read().unwrap());
